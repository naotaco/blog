...
で、WebP表示をブラウザから確認してみると、画像をクリックしたときのLightBox的な動き？にならずに、単なるファイルへのリンクとして動いているっぽい。PNG/JPGの場合はそれっぽく表示してくれるので、できれば同じように動いてほしかったが。フィードバックしてみるか、と思ったがとっくに修正済みで、テーマをアップデートすればいいらしい。
